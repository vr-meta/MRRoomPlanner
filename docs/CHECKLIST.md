...
- [~] G1. Корень: гравитация в Scan OFF роняла **риг** — отметка ног дрейфовала, каждый
  ...
  проверка в шлеме
- [~] G2. **Датум ног = пол скана** (фидбек: «вишу ~20-25 см над полом везде», Layout
  кладёт пол правильно): системная калибровка пола Quest ниже реального (стоячий лог:
  головa 1.82-1.9 при support=0), а Layout верит скану. `GroundService.FootY` = Y
  FLOOR-якоря MRUK (рефлексия, probe раз в 3 с до находки) с фолбэком на 0; settle/
  телепорт/CanStepTo — от FootY; стоячий лог со scanFloor. Тест ScannedFloor_Becomes
  TheFootDatum. Хвост: мультирум (ближайший якорь), шлем-проверка
