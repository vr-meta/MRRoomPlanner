...
- [ ] AB1. **Инструмент Projects** (#58): именованные проекты (список, New/Open/Save/
  Delete, имена автогенерацией — на Quest нет клавиатуры), автосейв на текущий проект;
  из Import убрать «New project» — импорт занимается только импортом
- [~] AB2. **Земля + гравитация + хождение** (#59, дизайн: `design/25-ground.md`):
  ...
  - [x] **G4 — Setup + тесты + доки**: `GroundService` в `SetupCoreRig`/`RigContext`/
    ...
    **EditMode 465/465, PlayMode 151/151**, синхронизированы `25-ground.md`,
    `21-locomotion.md`, `design/README.md`
  - [ ] **G5 — проверка на устройстве**: комфорт падения, ходьба по лестнице,
    отсутствие тошноты при вертикали (headless не видно)
- [ ] AB3. **Маркер места импорта** (#60): футпринт/кольцо цели до «Load IFC»,
  посадка модели в отмеченную точку на земле; без маркера — прежний одноклик
- [ ] AB4. **Тесты действий** (#61): расширить скриптованные жесты
  (`SelectGesturePlayTests`, шов из 7de0db9) на покраску, виджеты инспектора
  (степперы/слайдер/попапы/деструктив-холд), рисование стен и проводов, радиал,
  телепорт; каждый новый фикс взаимодействия — с таким регрессионным тестом
