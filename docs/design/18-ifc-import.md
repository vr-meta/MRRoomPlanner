...
3. **Маппинг (MVP):**
   ...
   | `IfcStairFlight` (подступёнки/проступи; Revit пишет размеры в футах) | параметрический `Stair` (Core), два типа: Solid (монолит) / Open (ступени на косоурах); ширина/направление — из bbox Brep-точек; ступени — цель телепорта. **Импорт по умолчанию делает Open** (монолит в MR читается как бетонная стена — фидбек 2026-08-10); выбор живёт в данных и переживает сохранение |
