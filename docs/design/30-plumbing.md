...
- `ProjectData.CurrentVersion = 6`: `ProjectPlumbFixture { Id, Kind, Angle, Position,
...
## Импорт из IFC (#118, 2026-08-15)

`IfcFlowSegment` с телом «экструзия круглого профиля» конвертируется в нативный
`PipeRoute`: центр/радиус — из кольца профиля, концы оси — через
`Placement × extra × Axis2Placement3D`, радиус → ближайший калибр (≥40 мм → D110,
≥22.5 мм → D50, иначе D40), вертикальные ходы длиннее 1 м — стояки. Спавн — через
фабрику персиста (`RestorePipe`), т.е. импортная труба неотличима от нарисованной.
Некруглые/Brep-сегменты и все `IfcFlowFitting` — бейк-меши (ничего не пропадает).
