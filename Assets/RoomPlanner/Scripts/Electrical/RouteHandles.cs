...
{
    /// <summary>
    /// Draggable control points of a wire route. Reuses the generic handle machinery
    /// (Phase B): the Select tool spawns visuals, picks and records undo — this only
    /// knows how to move a bend. Preview skips the collider re-cook (rule 4.2).
    /// </summary>
    [RequireComponent(typeof(WireRoute))]
    public class RouteHandles : MonoBehaviour, IHandleProvider
    {
        ...
        public ICommand CommitHandle(int index, Vector3 from, Vector3 to)
        {
            ...
            return new RouteBendMoveCommand(this, index, from, to);
        }
    ...
    public sealed class RouteBendMoveCommand : ICommand, ISelectableCommand
    {
        private readonly RouteHandles _handles;
        private readonly int _index;
        private readonly Vector3 _from, _to;

        public RouteBendMoveCommand(RouteHandles handles, int index, Vector3 from, Vector3 to)
        {
            _handles = handles; _index = index; _from = from; _to = to;
        }

        public string Name => "Move wire bend";
